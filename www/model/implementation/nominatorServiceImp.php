...
class nominatorServiceImp implements nominatorService
{
    ...
    function getNomineesRequiringApproval($sessionID)
    {
        // Retrieve access to the database.
        $db = db_connect();
        if (NULL == $db)
        ...
        try
        {
            ...
            {
                array_push($nomForms, new nominationForm($sessionID, $result['PID'], $result['NominatorID'], $result['FirstName'], $result['LastName'], $result['EmailAddress'], $result['Ranking'], $result['IsCSGradStudent'], $result['IsNewGradStudent'], $result['ApplicationReceived'], $result['ApplicationVerified'], $result['ExpectedGTAHours'], $result['Timestamp']));
            }
    ...
    function getNomineesThatNeverResponded($sessionID)
    {
        // Retrieve access to the database.
        $db = db_connect();
        if (NULL == $db)
